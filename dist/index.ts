...
class JWT {
    ...
    constructor(SECRET_KEY: string, ISS: string) {
         ...
         * @returns {String} - base64
         */
        this.buildPayload = (params: object): string => {
            try {
                const { enc } = this.props;
         ...
         * @returns {String} - base64
         */
        this.buildSignature = (prev_token: string): string => {
            try {
                const { enc } = this.props;
                const hash = CryptoJS.HmacSHA256(prev_token, SECRET_KEY);
         ...
         */
        this.checkJWT = (req: req): checkJWT => {
            try {
                const { enc } = this.props;
